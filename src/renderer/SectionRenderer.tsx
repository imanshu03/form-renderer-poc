...
import { SubSectionsRenderer } from "./SubSectionRenderer";
...
export function SectionRenderer({
  ...
  const renderContent = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        {hasDirectFields && renderDirectFields()}

        {/* Subsections with accordion UI */}
        ...
        {/* Empty state */}
        {!hasDirectFields && !hasSubsections && (
          <div className="text-sm text-muted-foreground">
            No fields or subsections defined for this section
          </div>
        )}
      </div>
    );
  ...
  // Render direct fields (non-subsection fields)
              ...
              return (
                ...
                >
                  {row.items.map((item, itemIndex) => {
                    ...
                    if (!field) {
                      console.warn(
                        `Field ${item.fieldId} not found in section ${section.id}`
                      );
                      return null;
                    }
  ...
  return (
    ...
    >
      ...
      <CardContent
        className={!section.label && !section.description ? "pt-6" : ""}
      >
        {renderContent()}
      </CardContent>
    </Card>
